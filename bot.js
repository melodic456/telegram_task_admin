...
var bot_token = '7467031593:AAESLgzJCQb2HNetjRGFLz9yt7ZQlCETeUk'; //YOUR BOT TOKEN HERE
...
const setTaskScene = new Scenes.BaseScene('set_task');
stage.register(setTaskScene)
const tasksListScene = new Scenes.BaseScene('tasks_list');
stage.register(tasksListScene)
const editTaskScene = new Scenes.BaseScene('edit_task');
stage.register(editTaskScene)
const deleteTaskScene = new Scenes.BaseScene('delete_task');
stage.register(deleteTaskScene)
const viewTaskScene = new Scenes.BaseScene('view_tasks');
stage.register(viewTaskScene)
const submitProofScene = new Scenes.BaseScene('submit_proof');
stage.register(submitProofScene)
const reviewSubmissionScene = new Scenes.BaseScene('review_submissions');
stage.register(reviewSubmissionScene)
      ...
      let admin = await db.collection('admindb').find({ admin: "admin" }).toArray()
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ctx.replyWithMarkdown(
                    '*🏡 Welcome To Main Menu*', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
...
bot.hears('💰 Balance', async (ctx) => {
    try {
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ctx.replyWithMarkdown(
                    '*🙌🏻 User = ' + ctx.from.first_name + '\n\n💰 Balance = ' + ub.toFixed(3) + ' ' + currency + '\n\n🪢 Invite To Earn More*', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
            } else {
                mustjoin(ctx)
            }
...
bot.hears('💰 Task', async (ctx) => {
    ctx.reply("Want a new Task", { parse_mode: 'markdown', reply_markup: { inline_keyboard: [[{ text: "✅ Yes", callback_data: "new_task_request" }]] } })
    ...
    //     if (bots == 'Active') {
    //         let userbalance = await db.collection('balance').find({ userID: ctx.from.id }).toArray()
    //         // get list of tasks for both all tasks and user tasks
    //         // find new task that is not done yet
    //         //  send reply like others
    //         // if replied with data check and add to database
    //         let task = await db.collection('task').find({ userID: ctx.from.id }).toArray()
    //         let ub = userbalance[0].balance
    //         let channel = admin[0].channels
    //         var flag = 0;
    //         for (i in channel) {
    ...
    //         }
    //         if (flag == channel.length) {
    //             db.collection('allUsers').updateOne({ userID: ctx.from.id }, { $push: { task: { task_id: 1, proof: "97398217", status: "pending"} } }, { upsert: true })
    //             ctx.replyWithMarkdown(
    //                 '*🙌🏻 User = ' + ctx.from.first_name + '\n\n💰 Balance = ' + ub.toFixed(3) + ' ' + currency + '\n\n🪢 Invite To Earn More*', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
    //             )
    //         } else {
...
bot.hears('🙌🏻 Invite', async (ctx) => {
    try {
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ctx.replyWithMarkdown(
                    '*🙌🏻 User =* [' + ctx.from.first_name + '](tg://user?id=' + ctx.from.id + ')\n\n*🙌🏻 Your Invite Link = https://t.me/' + ctx.botInfo.username + '?start=' + ctx.from.id + ' \n\n🪢 Invite To ' + refer + ' ' + currency + ' Per Invite*', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
            } else {
                mustjoin(ctx)
            }
        ...
        if (bots == 'Active') {
            if (withs == 'ON') {
                ...
                if (flag == channel.length) {
                    ...
                    if (ub < mini_with) {
                        ctx.replyWithMarkdown(
                            '*⚠️ Must Own AtLeast ' + mini_with + ' ' + currency + ' To Make Withdrawal*', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                        )
                    } else if (!data[0].wallet) {
                        ctx.replyWithMarkdown(
                            '*⚠️ Set Your Wallet Using : *`🗂 Wallet`', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                        )
                    } else {
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ...
                if (statdata.length == 0) {
                    ...
                    ctx.reply(
                        '<b>📊 Bot Live Stats 📊\n\n📤 Total Payouts : 0 ' + currency + '\n\n💡 Total Users: ' + members.length + ' Users\n\n🔎 Coded By: <a href="tg://user?id=1003376875">ROHIT_154</a></b>' , { parse_mode: 'html', reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                    )
                } else {
        ...
        ctx.replyWithMarkdown(
            '*🏡 Welcome To Admin Panel*\n\nHere are the available admin commands:',
            {
                reply_markup: {
                    inline_keyboard: [
                        [
                            { text: "💰 Change Refer", callback_data: "refer" },
                            { text: "💰 Change Minimum", callback_data: "minimum" }
                        ],
                        [
                            { text: "🤖 Bot Status", callback_data: "botstat" }
                        ],
                        [
                            { text: "🌲 Change Channels", callback_data: "channels" },
                            { text: "🎁 Change Bonus", callback_data: "bonus" }
                        ],
                        [
                            { text: "📤 Withdrawals Status", callback_data: "withstat" }
                        ],
                        [
                            { text: "🚹 User Details", callback_data: "userdetails" },
                            { text: "🔄 Change Balance", callback_data: "changebal" }
                        ],
                        [
                            { text: "✏️ Paytm Keys", callback_data: "keys" }
                        ],
                        [
                            { text: "➕ Set New Task", callback_data: "set_task" } // New button for setting a new task
                        ],
                        [
                            { text: "📋 Tasks List", callback_data: "tasks_list" } // New button for viewing tasks
                        ]
                    ]
                }
            }
...
bot.action('tasks_list', async (ctx) => {
    try {
        let tasks = await db.collection('tasks').find({}).toArray();
        let taskList = tasks.map((task, index) => `${index + 1}. ${task.text} - Reward: ${task.rewardAmount}`).join('\n');

        ctx.replyWithMarkdown(
            '*📋 All Tasks:*\n' + taskList, { reply_markup: { inline_keyboard: tasks.map((_, index) => [{ text: `Edit Task ${index + 1}`, callback_data: `edit_task_${index}` }, { text: `Delete Task ${index + 1}`, callback_data: `delete_task_${index}` }]) } }
        );
    } catch (error) {
        console.log(error);
    }
...
bot.action(/^edit_task_(\d+)$/, async (ctx) => {
    try {
        let taskIndex = parseInt(ctx.match[1]);
        let tasks = await db.collection('tasks').find({}).toArray();
        let selectedTask = tasks[taskIndex];

        ctx.session.taskIndex = taskIndex;
        ctx.session.taskDetails = selectedTask;

        ctx.replyWithMarkdown(
            '*📝 Edit Task:*\n\n' +
            `*Task Text:* ${selectedTask.text}\n` +
            `*Image URL:* ${selectedTask.imageUrl}\n` +
            `*Reward Amount:* ${selectedTask.rewardAmount}\n\n` +
            '*Please enter the new task details in the format:* `Task Text | Image URL | Reward Amount`',
            { reply_markup: { 
                inline_keyboard: [
                    [{ text: `Edit Task ${taskIndex + 1}`, callback_data: `editing_task_${taskIndex}` }]
                ],
                keyboard: [['⛔ Cancel']], 
        ...
        ctx.scene.enter('edit_task');
    } catch (error) {
        console.log(error);
    }
});

bot.hears(/^editing_task_(\d+)$/, async (ctx) => {
    ...
    try {
        if (ctx.message.text == '⛔ Cancel') {
            ...
            let details = ctx.message.text.split('|');
            let taskText = details[0];
            let imageUrl = details[1];
            let rewardAmount = parseFloat(details[2]);

            let taskIndex = ctx.session.taskIndex;
            let taskDetails = ctx.session.taskDetails;

            await db.collection('tasks').updateOne(
                { _id: taskDetails._id },
                { $set: { text: taskText, imageUrl: imageUrl, rewardAmount: rewardAmount } }
            );

            ctx.replyWithMarkdown(
                '*🗂 Task Updated Successfully: *\n`' + taskText + '`', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
            );
        }
        ctx.scene.leave('edit_task');
    } catch (error) {
        console.log(error);
    }
});
...
bot.hears(/^.+$/, async (ctx) => {
    try {
        ...
        } else if (ctx.session && ctx.session.taskIndex !== undefined) {
            let details = ctx.message.text.split('|');
            let taskText = details[0];
            let imageUrl = details[1];
            let rewardAmount = parseFloat(details[2]);
...
bot.action(/^delete_task_(\d+)$/, async (ctx) => {
    try {
        ...
        await db.collection('tasks').deleteOne({ _id: selectedTask._id });

        ctx.replyWithMarkdown('*🗑 Task Deleted Successfully*');
        ctx.scene.reenter();
    } catch (error) {
        console.log(error);
    }
...
broadcast.hears(regex, async (ctx) => {
    try {
        if (ctx.message.text == '⛔ Cancel') {
            ...
            ctx.replyWithMarkdown(
                '*📣 Broadcast Sent To: ' + users.length + ' Users*', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
            )
    ...
    try {
        ...
        if (flag == channel.length) {
            ...
            if (ctx.message.text == '⛔ Cancel') {
                ...
                ctx.replyWithMarkdown(
                    '*🗂 Wallet Address Set To: *\n`' + ctx.message.text + '`', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
            ...
            if (flag == channel.length) {
                ...
                if (ctx.message.text == '⛔ Cancel'){
                  ctx.replyWithMarkdown(
                        ...
                        '*⛔ Withdrawal Cancelled*', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                    ...
                    ctx.replyWithMarkdown(
                        '*⛔ Only Numeric Value Allowed*', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                    )
                    ctx.scene.leave('onWithdraw')
                    return 0;
                } else if (ctx.message.text > ub) {
                    ctx.replyWithMarkdown(
                        '*⛔ Entered Amount Is Greater Than Your Balance*', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                    )
                    ctx.scene.leave('onWithdraw')
                    return 0;
                    ...
                    ctx.replyWithMarkdown(
                        ...
                        '*⚠️ Minimum Withdrawal Is ' + mini_with + ' ' + currency + '*', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                  ...
                  ctx.replyWithMarkdown(
                        ...
                        '*⚠️ Maximum Withdrawal Is 10 ' + currency + '*', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
  ...
  try{
    ...
    if(toWith == 0){
      ...
      return 0;
        ...
        ctx.deleteMessage()
        ctx.replyWithMarkdown(
                        "*✅ New Withdrawal Processed ✅\n\n🚀Amount : " + toWith + " " + currency + "\n⛔ Wallet :* `" + wallet + "`\n*💡 Bot: @" + ctx.botInfo.username + "*", {parse_mode:'markdown', reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                    )
...
bot.action("cancel",async(ctx)=> {
  try{
     ...
     ctx.replyWithMarkdown(
                        ...
                        "*❌ Withdrawal Cancelled *", {parse_mode:'markdown', reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
...
tgid.hears(/^[0-9]+$/, async (ctx) => {
    try {
        if (ctx.message.text == '⛔ Cancel') {
            ...
            if (!data[0]) {
                ctx.replyWithMarkdown(
                    '*⛔ User Is Not Registered In Our Database *', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
            } else {
                ...
                ctx.reply(
                    '<b>🫂 User : <a href="tg://user?id=' + ctx.message.text + '">' + ctx.message.text + '</a>\n⛔ User Id</b> : <code>' + ctx.message.text + '</code>\n\n<b>💰 Balance : ' + bal + '\n🗂 Wallet : </b><code>' + add + '</code>\n<b>🙌🏻 Inviter : </b><code>' + invite + '</code>', { parse_mode: 'html', reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
            }
        }
...
incr.hears(regex, async (ctx) => {
    try {
        if (ctx.message.text == '⛔ Cancel') {
            ...
            ctx.reply(
                '<b>💰 Balance Of <a href="tg://user?id=' + user + '">' + user + '</a> Was Increased By ' + amount + '\n\n💰 Final Balance = ' + final + '</b>', { parse_mode: 'html', reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
            )
...
chnl.hears(regex, async (ctx) => {
    try {
        ...
        if (ctx.message.text == '⛔ Cancel') {
            ...
            ctx.reply(
                '<b>🗂 Channel Added To Bot : ' + ctx.message.text + '</b>', { parse_mode: 'html', reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
            )
        } else {
            ctx.replyWithMarkdown(
                '*⛔ Channel User Name Must Start With "@"*', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
            )
        }
    ...
    try {
        ...
        if (ctx.message.text == '⛔ Cancel') {
            ...
            if (contains("" + ctx.message.text + "", chan)) {
                ...
                ctx.reply(
                    '<b>🗂 Channel Removed From Bot : ' + ctx.message.text + '</b>', { parse_mode: 'html', reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
            } else {
                ctx.reply(
                    '<b>⛔ Channel Not In Our Database</b>', { parse_mode: 'html', reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
                )
            }
        } else {
            ctx.replyWithMarkdown(
...
paychnl.hears(regex, async (ctx) => {
    try {
        ...
        if (ctx.message.text == '⛔ Cancel') {
            ...
            ctx.reply(
                '<b>🗂 Pay Channel Set To : ' + ctx.message.text + '</b>', { parse_mode: 'html', reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
            )
        } else {
            ctx.replyWithMarkdown(
                '*⛔ Channel User Name Must Start With "@"*', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
...
function arrayRemove(arr, value) {
 ...
 // now need message to a user
// grab message text from a user

// bot.hears("New Task")

bot.action('set_task', async (ctx) => {
    try {
        ctx.deleteMessage();
        ctx.reply(
            '*💡 Enter Task Details (Text, Image URL, Reward Amount)*', { parse_mode: 'markdown', reply_markup: { keyboard: [['⛔ Cancel']], resize_keyboard: true } }
        );
        ctx.scene.enter('set_task');
    } catch (error) {
        console.log(error);
    }
});
...
bot.action('view_tasks', async (ctx) => {
    try {
        let tasks = await db.collection('tasks').find({ status: 'pending' }).toArray();
        let taskList = tasks.map((task, index) => `${index + 1}. ${task.text} - Reward: ${task.rewardAmount}`).join('\n');

        ctx.replyWithMarkdown(
            '*📋 Available Tasks:*\n' + taskList, { reply_markup: { keyboard: tasks.map((_, index) => [`${index + 1}`]), resize_keyboard: true } }
        );
    } catch (error) {
        console.log(error);
    }
...
bot.hears(/^\d+$/, async (ctx) => {
    try {
        let taskIndex = parseInt(ctx.message.text) - 1;
        let tasks = await db.collection('tasks').find({ status: 'pending' }).toArray();
        let selectedTask = tasks[taskIndex];

        ctx.replyWithPhoto(selectedTask.imageUrl, {
            caption: `*📋 Task Details:*\n${selectedTask.text}\n\n*💰 Reward: ${selectedTask.rewardAmount}*`,
            parse_mode: 'markdown',
            reply_markup: { keyboard: [['⛔ Cancel', '✅ Submit Proof']], resize_keyboard: true }
        });
    } catch (error) {
        console.log(error);
    }
...
bot.action('submit_proof', async (ctx) => {
    try {
        ctx.deleteMessage();
        ctx.reply(
            '*💡 Submit Your Proof (Text, Image URL)*', { parse_mode: 'markdown', reply_markup: { keyboard: [['⛔ Cancel']], resize_keyboard: true } }
        );
        ctx.scene.enter('submit_proof');
    } catch (error) {
        console.log(error);
    }
});
...
bot.hears(/^.+$/, async (ctx) => {
    try {
        if (ctx.message.text == '⛔ Cancel') {
            ...
            let details = ctx.message.text.split('|');
            let proofText = details[0];
            let proofImageUrl = details[1];
            ...
            ctx.replyWithMarkdown(
                '*🗂 Proof Submitted Successfully: *\n`' + proofText + '`', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
            );
        }
        ctx.scene.leave('submit_proof');
    } catch (error) {
        console.log(error);
    }
});

bot.action('review_submissions', async (ctx) => {
    try {
        let submissions = await db.collection('submissions').find({ status: 'pending' }).toArray();
        let submissionList = submissions.map((submission, index) => `${index + 1}. User: ${submission.userID}\nProof: ${submission.proofText}`).join('\n');

        ctx.replyWithMarkdown(
            '*📋 Pending Submissions:*\n' + submissionList, { reply_markup: { keyboard: submissions.map((_, index) => [`${index + 1}`]), resize_keyboard: true } }
        );
    } catch (error) {
        console.log(error);
    }
...
bot.hears(/^\d+$/, async (ctx) => {
    try {
        let submissionIndex = parseInt(ctx.message.text) - 1;
        let submissions = await db.collection('submissions').find({ status: 'pending' }).toArray();
        let selectedSubmission = submissions[submissionIndex];

        ctx.replyWithPhoto(selectedSubmission.proofImageUrl, {
            caption: `*📋 Submission Details:*\nUser: ${selectedSubmission.userID}\nProof: ${selectedSubmission.proofText}`,
            parse_mode: 'markdown',
            reply_markup: { keyboard: [['⛔ Decline', '✅ Accept']], resize_keyboard: true }
        });
    } catch (error) {
        console.log(error);
    }
...
bot.hears(/^(⛔ Decline|✅ Accept)$/, async (ctx) => {
    try {
        let decision = ctx.message.text;
        let submissionIndex = parseInt(ctx.message.text.split(' ')[1]) - 1;
        let submissions = await db.collection('submissions').find({ status: 'pending' }).toArray();
        let selectedSubmission = submissions[submissionIndex];

        if (decision == '✅ Accept') {
            db.collection('submissions').updateOne({ _id: selectedSubmission._id }, { $set: { status: 'accepted' } });
            db.collection('balance').updateOne({ userID: selectedSubmission.userID }, { $inc: { balance: selectedSubmission.rewardAmount } });
            ctx.replyWithMarkdown(
                '*✅ Submission Accepted: *\nUser: `' + selectedSubmission.userID + '`', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
            );
        } else {
            db.collection('submissions').updateOne({ _id: selectedSubmission._id }, { $set: { status: 'declined' } });
            ctx.replyWithMarkdown(
                '*⛔ Submission Declined: *\nUser: `' + selectedSubmission.userID + '`', { reply_markup: { keyboard: [['📋 View Tasks'], ['💰 Balance'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['💳 Withdraw', '📊 Statistics']], resize_keyboard: true } }
            );
        }
