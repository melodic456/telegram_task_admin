...
const addWalletType = new BaseScene('addWalletType')
stage.register(addWalletType)

const removeWalletType = new BaseScene('removeWalletType')
stage.register(removeWalletType)

const selectWallet = new BaseScene('selectWallet')
stage.register(selectWallet)
...
// Add wallet type scene
addWalletType.enter(async (ctx) => {
    ctx.reply('Enter new wallet type name (e.g., BTC, ETH, USDT):', 
        { reply_markup: { keyboard: [['⛔ Cancel']], resize_keyboard: true } }
    )
})

addWalletType.on('text', async (ctx) => {
    if (ctx.message.text === '⛔ Cancel') {
        ctx.reply('Operation cancelled', 
            { reply_markup: { keyboard: [['💰 Balance'], ['🙌🏻 Invite', '🗂 Wallet'], ['💳 Withdraw', '📝 View Tasks']], resize_keyboard: true } }
        )
        return ctx.scene.leave()
    }

    const walletType = ctx.message.text.toUpperCase()
    const admin = await db.collection('admindb').findOne({ admin: "admin" })
    
    let walletTypes = admin.walletTypes || []
    if (walletTypes.includes(walletType)) {
        ctx.reply('This wallet type already exists!')
        return ctx.scene.leave()
    }
    ...
    await db.collection('admindb').updateOne(
        { admin: "admin" },
        { $set: { walletTypes: walletTypes } }
    )

    ctx.reply(`Wallet type ${walletType} has been added successfully!`,
        { reply_markup: { keyboard: [['💰 Balance'], ['🙌🏻 Invite', '🗂 Wallet'], ['💳 Withdraw', '📝 View Tasks']], resize_keyboard: true } }
    )
    return ctx.scene.leave()
})

// Remove wallet type scene
removeWalletType.enter(async (ctx) => {
    const admin = await db.collection('admindb').findOne({ admin: "admin" })
    const walletTypes = admin.walletTypes || []
    
    if (walletTypes.length === 0) {
        ctx.reply('No wallet types to remove!',
            { reply_markup: { keyboard: [['💰 Balance'], ['🙌🏻 Invite', '🗂 Wallet'], ['💳 Withdraw', '📝 View Tasks']], resize_keyboard: true } }
        )
        return ctx.scene.leave()
    }

    const keyboard = walletTypes.map(type => [type])
    keyboard.push(['⛔ Cancel'])
    
    ctx.reply('Select wallet type to remove:',
        { reply_markup: { keyboard: keyboard, resize_keyboard: true } }
    )
})

removeWalletType.on('text', async (ctx) => {
    if (ctx.message.text === '⛔ Cancel') {
        ctx.reply('Operation cancelled',
            { reply_markup: { keyboard: [['💰 Balance'], ['🙌🏻 Invite', '🗂 Wallet'], ['💳 Withdraw', '📝 View Tasks']], resize_keyboard: true } }
        )
    ...
    if (walletTypes.includes(ctx.message.text)) {
        walletTypes = walletTypes.filter(type => type !== ctx.message.text)
        await db.collection('admindb').updateOne(
            { admin: "admin" },
            { $set: { walletTypes: walletTypes } }
        )
        ctx.reply(`Wallet type ${ctx.message.text} has been removed!`,
            { reply_markup: { keyboard: [['💰 Balance'], ['🙌🏻 Invite', '🗂 Wallet'], ['💳 Withdraw', '📝 View Tasks']], resize_keyboard: true } }
        )
    } else {
        ctx.reply('Invalid wallet type!',
            { reply_markup: { keyboard: [['💰 Balance'], ['🙌🏻 Invite', '🗂 Wallet'], ['💳 Withdraw', '📝 View Tasks']], resize_keyboard: true } }
        )
    }
    return ctx.scene.leave()
...
// Modify existing wallet scene
// wallet.enter(async (ctx) => {
//     const admin = await db.collection('admindb').findOne({ admin: "admin" })
//     const walletTypes = admin.walletTypes || []
    
//     if (walletTypes.length === 0) {
//         ctx.reply('No wallet types available. Please contact admin.',
//             { reply_markup: { keyboard: [['💰 Balance'], ['🙌🏻 Invite', '🗂 Wallet'], ['💳 Withdraw', '📝 View Tasks']], resize_keyboard: true } }
//         )
//         return ctx.scene.leave()
//     }

//     ctx.session.walletTypes = walletTypes
//     const keyboard = walletTypes.map(type => [type])
//     keyboard.push(['⛔ Cancel'])
    
//     ctx.reply('Select wallet type:',
//         { reply_markup: { keyboard: keyboard, resize_keyboard: true } }
//     )
//     return ctx.scene.enter('selectWallet')
// })

// Select wallet type scene
selectWallet.on('text', async (ctx) => {
    if (ctx.message.text === '⛔ Cancel') {
        ctx.reply('Operation cancelled',
            { reply_markup: { keyboard: [['💰 Balance'], ['🙌🏻 Invite', '🗂 Wallet'], ['💳 Withdraw', '📝 View Tasks']], resize_keyboard: true } }
        )
    ...
    const walletType = ctx.message.text
    if (!ctx.session.walletTypes.includes(walletType)) {
        ctx.reply('Invalid wallet type! Please select from the options.',
            { reply_markup: { keyboard: [['💰 Balance'], ['🙌🏻 Invite', '🗂 Wallet'], ['💳 Withdraw', '📝 View Tasks']], resize_keyboard: true } }
        )
        return ctx.scene.leave()
    }

    ctx.session.selectedWalletType = walletType
    ctx.reply(`Please enter your ${walletType} wallet address:`,
        { reply_markup: { keyboard: [['⛔ Cancel']], resize_keyboard: true } }
    )
...
// Modify existing wallet scene handler
wallet.on('text', async (ctx) => {
    try {
        ...
        const walletType = ctx.session.selectedWalletType
        const walletAddress = ctx.message.text

        // Update user's wallets in the database
        ...
        ctx.reply(
            `✅ Your ${walletType} wallet has been set to:\n${walletAddress}`,
            { reply_markup: { keyboard: [['💰 Balance'], ['🙌🏻 Invite', '🗂 Wallet'], ['💳 Withdraw', '📝 View Tasks']], resize_keyboard: true } }
        )
        return ctx.scene.leave()
    } catch (error) {
...
// Modify the admin panel to include wallet type management
bot.action('walletTypes', async (ctx) => {
    const admin = await db.collection('admindb').findOne({ admin: "admin" })
    const walletTypes = admin.walletTypes || []
    
    let walletList = walletTypes.length > 0 
        ? walletTypes.join('\n') 
        : 'No wallet types configured'
    ...
    ctx.editMessageText(
        `Current Wallet Types:\n\n${walletList}`,
        {
            reply_markup: {
                inline_keyboard: [
                    [
                        { text: "➕ Add Wallet Type", callback_data: "addWalletType" },
                        { text: "➖ Remove Wallet Type", callback_data: "removeWalletType" }
                    ],
                    [{ text: "🔙 Back", callback_data: "adminBack" }]
                ]
            }
        }
    )
...
bot.action('addWalletType', (ctx) => {
    ctx.deleteMessage()
    ctx.scene.enter('addWalletType')
})

bot.action('removeWalletType', (ctx) => {
    ctx.deleteMessage()
    ctx.scene.enter('removeWalletType')
})

// Modify the wallet button handler to show multiple wallets
    ...
    try {
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                const user = await db.collection('allUsers').findOne({ userID: ctx.from.id })
                const walletTypes = admin[0].walletTypes || []
                
                let message = '💼 Your Wallets:\n\n'
                if (user && user.wallets) {
                    for (const type of walletTypes) {
                        message += `${type}: ${user.wallets[type] || 'Not Set'}\n`
                    }
                } else {
                ...
                ctx.reply(message, {
                    ...
                    reply_markup: {
                        inline_keyboard: [[
                            { text: "🔄 Update Wallet", callback_data: "wallet" }
                        ]]
                    }
                })
            } else {
